...
class UserDashboardComponent extends Component
{
    public function render()
    {
        ...
        $totalDelivered = Order::where('status','!=','canceled')->where('user_id',Auth::user()->id)->count();
        $totalCanceled = Order::where('status','!=','delivered')->where('user_id',Auth::user()->id)->count();
