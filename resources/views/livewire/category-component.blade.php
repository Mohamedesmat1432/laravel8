    ...
    <div class="container mt-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb px-4 py-3">
                ...
                <li class="breadcrumb-item active bold" aria-current="page">{{ __($category_name)}}</li>
            </ol>
        </nav>
        <div class="row p-3">
                    ...
                    <div class="card-body" >
                        ...
                        <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4">
                            @foreach ($categories as $category)
                                <li class="nav-item {{(count($category->subCategories) > 0) ?'has-child-cat':''}}">
                                    <a href="{{route('product.category',['category_slug'=>$category->slug])}}" class="{{(request()->is(LaravelLocalization::setLocale(). '/product-category'.'/'.$category->slug)) ? 'active' : '' }} {{__('messages.float')}} hvr-grow">
                                    ...
                                    @if (count($category->subCategories) > 0)
                                        ...
                                        <ul class="sub-cate" style="display: none">
                                            @foreach ($category->subCategories as $scategory)
                                                <li class="nav-item">
                                                    <a href="{{route('product.category',['category_slug'=>$category->slug,'scategory_slug'=>$scategory->slug])}}" class="{{(request()->is(LaravelLocalization::setLocale(). '/product-category'.'/'.$category->slug.'/'.$scategory->slug)) ? 'active' : '' }} {{__('messages.float')}} hvr-grow">
                                                        <i class="bi bi-dash"></i>
                                                        {{$scategory->name}}
                                                    </a>
                                                </li>
            ...
            <div class="col-md-8 ">
                @if ($products->count() > 0)
                    <div class="card bg-card">
                        <div class="card-body">
                            <div class="row gx-3 gx-lg-4 row-cols-2 row-cols-md-3 row-cols-xl-3 justify-content-center mb-3">
                                                ...
                                                <div class="text-center">
                                                    ...
                                                    <div class="product-wish">
                                                        ...
                                                        @if ($wishItems->contains($product->id))
                                                            <a href="#" wire:click.prevent="removeFromWishlist({{$product->id}})"><i class="bi-heart-fill fill-star"></i></a>
                                                        @else
                                                            <a href="#" wire:click.prevent="addToWishList('{{$product->id}}','{{$product->name}}','{{$product->regular_price}}')"><i class="bi-heart-fill "></i></a>
                                                        @endif
                                                    </div>
                                                    <!-- Product price-->
                ...
                @else
                    <div class="container">
                        <h5 class="text-center p-5 text-danger">
